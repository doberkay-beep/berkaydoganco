...
export function MediaContent({ lang }: { lang: Lang }) {
  ...
  return (
    <div style={{ paddingTop: "60px" }}>
      <style>{`
        .media-link { transition: color 0.25s ease, border-color 0.25s ease; }
        .media-link:hover { color: var(--accent) !important; border-color: var(--bordo) !important; }
        .embed-frame { border: 1px solid var(--dim); border-radius: 2px; overflow: hidden; background: var(--bg2); }
      ...
      {/* HERO */}
      <section className="media-hero" style={{ minHeight: "45vh", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "4rem 2.75rem", borderBottom: "1px solid var(--border)" }}>
        <p className="eyebrow" style={{ marginBottom: "2rem" }}>{t.kicker}</p>
        <h1 style={{ fontFamily: "var(--font-grotesk)", fontWeight: 300, fontSize: "clamp(3rem, 9vw, 7rem)", lineHeight: 0.95, letterSpacing: "-0.035em", color: "var(--cream)", marginBottom: "1.25rem", whiteSpace: "pre-line" }}>{t.title}</h1>
        <p style={{ fontFamily: "var(--font-grotesk)", fontStyle: "italic", fontWeight: 300, fontSize: "1.25rem", color: "var(--gray)", maxWidth: "460px", lineHeight: 1.6 }}>{t.desc}</p>
      </section>

      {/* PODCAST — Spotify */}
      <section className="media-sec" style={{ padding: "5rem 2.75rem", borderBottom: "1px solid var(--border)" }}>
        <p className="eyebrow" style={{ marginBottom: "1.25rem" }}>{t.podcastKicker}</p>
      ...
      {/* YOUTUBE */}
      <section className="media-sec" style={{ padding: "5rem 2.75rem", borderBottom: "1px solid var(--border)" }}>
        <p className="eyebrow" style={{ marginBottom: "1.25rem" }}>{t.videoKicker}</p>
      ...
      {/* PLATFORM LİNKLERİ */}
      <section className="media-sec" style={{ padding: "4rem 2.75rem", textAlign: "center" }}>
